...
Toggles: =SPC t i/I= estaban pillados (indent-bars y el instalador de fuentes),
asi que van en =h/H=:
- =SPC t h= : on/off en ESTE buffer.
- =SPC t H= : on/off GLOBAL (todos los buffers con LSP).

Nota: intelephense aqui solo envia hints de *parametro* (los de *tipo* no los
manda), por eso el segundo eje es global en vez de "solo-params / +tipos".
...
(defun my/inlay-hints-toggle ()
  "On/off de los inlay hints en el buffer actual."
  (interactive)
  (if (bound-and-true-p lsp-inlay-hints-mode)
      (progn (lsp-inlay-hints-mode -1) (message "Inlay hints: OFF (buffer)"))
    (lsp-inlay-hints-mode 1)
    (message "Inlay hints: ON (buffer)")))

(defun my/inlay-hints-toggle-global ()
  "On/off de los inlay hints en TODOS los buffers con LSP."
  (interactive)
  ...
  (message "Inlay hints GLOBAL: %s" (if lsp-inlay-hint-enable "ON" "OFF")))

(map! :leader
      :desc "Inlay hints (buffer)" "t h" #'my/inlay-hints-toggle
      :desc "Inlay hints (global)" "t H" #'my/inlay-hints-toggle-global)
#+end_src
